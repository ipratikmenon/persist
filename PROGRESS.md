...
behind it. They are now a `list` with a table around it and a total under it,
and the total is **summed from the rows**. A demand notice that states a sum and
then schedules the payments making it up has said the same thing twice, and the
day the two disagree is the day the notice is worth arguing about.

  ✅ `PAYMENTS_BLOCK` — date, amount, mode (UPI / bank transfer / cheque / cash
     / card) and a reference for the UTR, cheque number or annexure.
  ✅ `blockTemplate` on a list — how the rows are wrapped, **once**, when there
     is at least one. A table needs its header row once, not once per row, and
     an empty schedule must print nothing rather than column headings over blank
     paper with a total of zero under them.
  ✅ `totalOf` — the numeric column `{{TOTAL}}` sums.
  ✅ Cells are set according to what the manifest says they are: the totalled
     column as money, a `date` cell as "11 February 2026". Otherwise a schedule
     shows "85000" against a total of "1,04,000.00" and reads its dates like a
     spreadsheet.

**Two shared formatters, for the same reason each.** `services/money.rs` came
out of billing so the invoice and the notice cannot group a figure differently;
`services/dates.rs` came out of firm.rs so a notice dated "14th July 2026" does
not sit over a schedule dated "2026-02-11".

**Drift checking extends to both:** a wrapper with no `{{ROWS}}` (every row
dropped), a `{{TOTAL}}` with nothing to total, a total of a column the row does
not have, and a total of a column that is not a number (which is zero, silently).

### The harness had a stale copy of a shipped manifest

`screenshots/mock/core.ts` had the real manifests **pasted into it**. Adding a
field to the notice left the harness drawing a form the app no longer has, and
the capture failed looking for a button that exists. They are imported now, so
the screenshots are always of the real thing.

Also: Playwright lives in a scratch install at `/tmp/shotkit` and went with the
container. `capture.mjs` now says how to put it back rather than failing with a
...
| Aug 2026 | `inputOnly` rather than a looser drift check | A field can be collected without being printed — the grounds selector drives a computed paragraph. Loosening the check to accommodate it would have lost the guarantee for every other field |
| Aug 2026 | Draft renders still validate | A preview showing a document that could never be filed teaches the attorney to trust it |
| Aug 2026 | `PERSIST_REQUIRE_LATEX` makes CI fail on a skipped engine test | A suite that skips its most important tests reports success while proving nothing — which is how three fatal faults reached a branch. Grepping output does not work; cargo captures test stdout |
...
| Aug 11 2026 | S27: the schedule of payments, and a figure nobody types. The notice's payment particulars were the last `computed` block with nothing behind it — now a `list` of tranches (date, amount, mode, reference) with a table around it and a total under it, **summed from the rows**: a demand notice that states a sum and separately schedules the payments making it up has said the same thing twice. New on a list: `blockTemplate` (how the rows are wrapped, once — a table needs its header once, not once per row, and an empty schedule must print nothing at all) and `totalOf` (the column `{{TOTAL}}` sums). Cells are set according to what the manifest says they are — the totalled column as money, a `date` cell as "11 February 2026" — so a schedule does not show "85000" against a total of "1,04,000.00". **Two shared formatters extracted:** `services/money.rs` out of billing and `services/dates.rs` out of firm.rs, each because two implementations of how this firm writes a figure or a date would drift. Drift checking covers a wrapper that would drop every row and a total of a column that is not a number. **The screenshot harness had the shipped manifests pasted into it** and went stale the moment a field was added — imported now | src-tauri/src/services/{templates,firm,money (new),dates (new)}.rs, src-tauri/src/commands/billing.rs, storage/templates/{legal-notice.{tex,json},_shared/persist-letterhead.tex}, src/lib/ipc-types.ts, screenshots/{mock/core.ts,capture.mjs} | cargo test: 324/324 with PERSIST_REQUIRE_LATEX=1 (was 311), Deck build PASS, 23 screenshots |
